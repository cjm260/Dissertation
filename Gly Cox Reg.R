...
#################
#CORONARY ATHEROSCLEROSIS
#Example Sex Stratified Cox Reg for Glycine
#Covariate file
...
df_pheno <- df_pheno %>%
  ...
  rename(idno = eid, PHENOTYPE = PHECODE_AgeAsTimescale, CENSOR_AGE = PHECODE_AgeAsTimescale_Years)
...
# Only analyzing Gly
pgs_full_Gly <- pgs_full %>%
  select(IID, gly)

# Reduced model PGS File for Gly
pgs_reduced <- fread("/rds/project/jmmh2/rds-jmmh2-projects/inouye_lab_other/impute_genomics/omics_age_sex/workingdirectory/Gly/UKB_Nightingale_Gly.sscore.gz")
pgs_reduced <- pgs_reduced %>%
  rename(gly = score_sum)
...
run_cox <- function(df, model_name) {
  cox_model <- coxph(Surv(CENSOR_AGE, PHENOTYPE) ~ adjusted_score + age_at_recruitment, data = df)
  ...
  df_result <- data.frame(Regression = model_name, Trait = "Gly", HR = hr, HR_low = hr_low, HR_high = hr_high, pvalue = pvalue, zscore = zscore, cindex = c_index, N_Case = n_cases, N_All = n_all)
  return(df_result)
}
...
prepare_data <- function(df_pgs, df_pheno_all) {
  ...
  df_pheno_test <- df_pheno_test %>%
    select(IID, all_of(score), sex, PHENOTYPE, CENSOR_AGE, age_at_recruitment, genotyping.array, paste0('PC', 1:10))
...
df_pheno_test_full <- prepare_data(pgs_full_Gly, df_pheno_all)
df_pheno_test_reduced <- prepare_data(pgs_reduced, df_pheno_all)


# Split data into male and female
df_male_full <- subset(df_pheno_test_full, sex == 0)
nrow(df_male_full) #264130
df_female_full <- subset(df_pheno_test_full, sex == 1)
nrow(df_female_full) #223022
df_male_reduced <- subset(df_pheno_test_reduced, sex == 0)
nrow(df_male_reduced) #264130
df_female_reduced <- subset(df_pheno_test_reduced, sex== 1)
nrow(df_female_reduced) #223022

# Run Cox regressions and save results
df_save <- rbind(df_save, run_cox(df_male_full, "Male Full"))
df_save <- rbind(df_save, run_cox(df_female_full, "Female Full"))
df_save <- rbind(df_save, run_cox(df_male_reduced, "Male Reduced"))
df_save <- rbind(df_save, run_cox(df_female_reduced, "Female Reduced"))


cox_model <- coxph(Surv(CENSOR_AGE, PHENOTYPE) ~ adjusted_score + age_at_recruitment, data = df_male_full)

# Write results to file
write.table(df_save, file = "/home/cjm260/pt3_Cox_results/Gly_PGS_411.4.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
# Write results to file
write.table(df_save, file = "/home/cjm260/pt3_Cox_results/Gly_PGS_411.1.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
# Write results to file
write.table(df_save, file = "/home/cjm260/pt3_Cox_results/Gly_PGS_401.1.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
